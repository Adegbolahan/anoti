...
- **Eight lifecycle hooks:** retrieval with a provenance trust boundary,
  ...
  reversibly via `anoti feedback`), a consolidation gate, and a
  **review-debt ledger** (`anoti review-debt`): adversarial review owed
  on a filed spec or a ready-for-review batch is tracked, aged in the
  digest, blocks the Stop hook once and makes integration ask until it
  is closed by a verdict or deferred with a written reason.
