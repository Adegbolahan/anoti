...
2. Set the session-state episode to `awaiting-approval` before any store
   write (the inhibition hook requires it — memory-organ writes outside an
   active consolidation flow are denied).
...
5. After writes: `scripts/regen-index` and `scripts/trust` on each touched
   store; promote surviving doubts to `open_questions`; update TODOS.md
   and LESSONS-LEARNT.md; set episode `committed`.
